...
export function panelPose(index, camera) {
  ...
  // A station has one world depth. Relative CSS depth changes on both sides
  // of the stop, just as the fixed WebGL tunnel does when the camera moves.
  const relativeDepth = (cameraDepth(index) - cameraDepth(camera)) * 26;
  const leaving =
    index === 0
      ? smoothstep((camera - 0.38) / 0.48)
      : smoothstep((camera - index - 0.08) / 0.72);
  return {
    // The close foreground uses a gentler depth scale, preserving the intro
    // long enough to read while the portal grows much faster behind it.
    z: index === 0 ? (cameraDepth(0) - cameraDepth(camera)) * 4 : relativeDepth,
    y: 0,
    rotateX: 0,
    exit: leaving,
